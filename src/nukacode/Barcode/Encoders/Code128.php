...
class Code128 extends BaseEncoder {

    public $code128_array   = [
     ...
     */
    protected $validationRegEx = '/^[\000-\177]*$/';

    public function encode($text)
    {
        $chksum = 104;

        $code_keys   = array_keys($this->code128_array);
        $code_values = array_flip($code_keys);
        $code_string = "";
        for ($X = 1; $X <= strlen($text); $X++) {
            $activeKey = substr($text, ($X - 1), 1);
            $code_string .= $this->code128_array[$activeKey];
            $chksum = ($chksum + ($code_values[$activeKey] * $X));
        }
        $code_string .= $this->code128_array[$code_keys[($chksum - (intval($chksum / 103) * 103))]];
